...
select.focus();
console.log(select.selectOnchange);
select.selectOnchange = selectOnchange
console.log(select.selectOnchange);
...
if (document.location.hash != "") {
  for (let a in s_array)
    if ("#" + s_array[a].value == document.location.hash) {
...
document
  .querySelector("header>a:first-of-type")
  .addEventListener("click", (e) => {
    e.preventDefault();
    move();
  });
document
  .querySelector("header>a:last-of-type")
  .addEventListener("click", (e) => {
    e.preventDefault();
    move(1);
  });
